...
void draw() {
  ...
  // ------ deposition in sequence ------
  // - rate of moving head
  // -
  // - iterate through the loop
  // - iterate through the sequences
  
  //verify that we are up to date with sequences
  if (!sequences.isEmpty() && sequences.size() > currentSequence) {
    
    // iterate through the sequences, the layers and the positions of centroids
    if (dist(headPosXSmooth,headPosYSmooth, headPosX, headPosY) < 1){
      setHeadPosition(sequences.get(currentSequence).centroids.get(currentPosition).x, sequences.get(currentSequence).centroids.get(currentPosition).y);
      currentPosition++;
      if (currentPosition >= sequences.get(currentSequence).centroids.size()){
        currentPosition = 0;
        currentLayer++;
        if (currentLayer >= sequences.get(currentSequence).nbOfLayers){
          currentLayer = 0;
          currentSequence++;
        }
      } 
...
void oscEvent(OscMessage theOscMessage) {
  ...
  // Receiving sequences centroids 
  if (theOscMessage.checkAddrPattern("/lcp/control/positions")) {
    //check if the typetag is the right one.
    println("yooo");
    //if (theOscMessage.checkTypetag("f")) { // might not work
    // parse theOscMessage and extract the values from the osc message arguments.
    ArrayList<PVector> centroids = new ArrayList<PVector>();

    println(" typetag: "+theOscMessage.typetag());
    println(" size of type tag: "+theOscMessage.typetag().length());
    println(" arguments size "+theOscMessage.arguments().length);
    int l = theOscMessage.typetag().length();
    for (int i = 0; i < (l-1)/3; i+=3) {
      float cX = theOscMessage.get(0+i).floatValue();
      float cY = theOscMessage.get(1+i).floatValue();
      float cZ = theOscMessage.get(2+i).floatValue();
      centroids.add(new PVector(cX, cY, cZ));
    }

    // get the amount of layers to deposit
    int nbLayers = theOscMessage.get(l-1).intValue();
    sequences.add(new Sequence(centroids, nbLayers));
